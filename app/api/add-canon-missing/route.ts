...
export async function GET() {
  try {
    ...
    if (!brand) return NextResponse.json({ error: 'canon brand not found' }, { status: 404 });
    ...
    // Remove duplicate canon-sf400 if it still exists
    const deleted = await Product.findOneAndDelete({ slug: 'canon-sf400' });

    return NextResponse.json({ ok: true, upserted: results, deleted: deleted?.slug ?? null });
  } catch (err: unknown) {
