...
export async function GET() {
  try {
    ...
    if (!brand) return NextResponse.json({ error: 'Contex brand not found' }, { status: 404 });

    const results = [];
    for (const p of products) {
      const doc = { ...p, brand: brand._id, brandSlug: 'contex' };
      const result = await Product.findOneAndUpdate(
        { slug: p.slug },
        { $set: doc },
        { upsert: true, returnDocument: 'after' }
      );
      results.push({ name: p.name, slug: p.slug, id: result?._id });
    }
