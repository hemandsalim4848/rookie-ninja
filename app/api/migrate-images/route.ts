...
export async function POST(req: Request) {
  const { secret, skip = 0, batchSize = 10, brandSlug } = await req.json()
  ...
  const query: any = { images: { $exists: true, $ne: [] } }
  if (brandSlug) query.brandSlug = brandSlug

  const products = await Product.find(query).skip(skip).limit(batchSize)
